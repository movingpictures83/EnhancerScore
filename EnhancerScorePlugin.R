...
model_file = paste(pfix, parameters["model_file", 2], sep="/")#options$DMR_epimark_file
##Epimark file of annotated regions
##Sample in which prediction is made
query_sample = parameters["query_sample", 2]
ref_sample = NULL
...
##Epimark file of DMRs
DMR_epimark_file = paste(pfix, parameters["DMR_epimark_file", 2], sep="/")#options$annotated_region_epimark_file
##Epimark file of input regions
query_region_epimark_file = paste(pfix, parameters["query_region_epimark_file", 2], sep="/")#options$annotated_region_epimark_file
...
incl_dev = 0
  if ("incl_dev" %in% rownames(parameters)) {
  incl_dev  <<- as.integer(parameters["incl_dev", 2])
  }
##Number of precessors to use
num_procs = 1
##Number of splits
num_splits <<- 1
  if ("num_splits" %in% rownames(parameters)) {
  num_splits  <<- as.integer(parameters["num_splits", 2])
	  print(as.integer(parameters["num_splits", 2]))
  }
print(num_splits)
##Whether to combine score of query regions and the scores from overlapping DMRs
not_genome_wide = 0#options$not_genome_wide
